...
class2value = {0: 0, 1: 20, 2: 60, 3: 40} #分为三大类，紫云英，背景类，油菜花

def postprocess(num=0):
  # switch region
  rows = 2544
  cols = 7194
  filenames = os.listdir('E:/实习/农作物分类/农作物分类代码/农作物分析代码/self_training')
  filepaths = [os.path.join('E:/实习/农作物分类/农作物分类代码/农作物分析代码/self_training', name) for name in filenames]
  Region = np.zeros(shape=(rows*7, cols*7), dtype=np.int16)
  for filepath in tqdm(filepaths, desc='Switching Region'):
    data = np.load(filepath).astype(np.int16)
    coord = data['coord']
  ...
  assert Region.shape==(12812,21889)
