...
class TrainDataset(data.Dataset):
  ...
  def __init__(self):
    ...
    self.root_path = './dataset_1/train_1' #self_training_2.py
		#========================================== 
  ...
  def __getitem__(self,idx):
    ...
    std = np.std(image,axis=(0,),keepdims=True)
    ...
    if np.random.uniform()>0.5:
      # random flip updown（上下）
      image = image[:,::-1,:]
    if np.random.uniform()>0.5:
      # random flip left-right（左右）
      image = image[:,:,::-1]
    #有一半的几率对图像进行上下翻转否则保持不变
